...
impl Iterator for Lexer {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while let Some(c) = self.advance() {
                ...
                '`' => {
                    ...
                    if c == '\\' {
                        ...
                        c = match self.esc_to_char(cha) {
                            Ok(c) => c,
                            Result::Err(e) => return e.into_token().some(),
                        };
                    }
                        ...
                        match c {
                            '\\' => {
                                if let Result::Err(e) = self.esc_to_char(c).map(|c| lit.push(c)) {
                                    return e.into_token().some();
                                }
                                continue;
                            },
                            '"' => return self.to_span().into_token(StrLit(lit)).some(),
                            _ => lit.push(c),
                        }
                    }
            ...
            self.nl = 0;
            return self.to_span().into_token(token).some();
        }
        None
    }
}
...
impl Lexer {
    ...
    fn advance(&mut self) -> Option<char> {
        self.next_char().or_else(|| self.next_line().and_then(|_| self.advance()))
    }
    ...
    fn next_line(&mut self) -> Option<()> {
        ...
        match self.file.read_until(b'\n', &mut line) {
            ...
            Ok(_) => (),
            Err(e) => {
                self.to_span().into_log().msg(format!("line get err: {}", e)).print();
                return None;
            },
        }
        ...
        match String::from_utf8(line) {
            ...
            Err(e) => {
                self.to_span().into_log().msg("Invalid utf8 in file").notes(e).print();
                None
            },
        }
    }
